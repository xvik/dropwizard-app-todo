 ...
 * @since 11.09.2025
 */
@TestGuiceyApp(TodoApp.class)
public class LightweightResourceTest {
    ...
    void testRest() {
        ...
        List<Todo> res = client.request("todo/").buildGet().invoke(new GenericType<>() {});

        // THEN list is empty
        assertThat(res).isEmpty();

        WHEN("Add element");
        Todo added = client.post("todo/", Todo.builder().title("one").completed(false).build(), Todo.class);
        ...
        added = client.post("todo/", Todo.builder().title("two").completed(false).build(), Todo.class);
        ...
        res = client.request("todo/").buildGet().invoke(new GenericType<>() {});
        ...
        WHEN("Loading by ID");
        Todo item = client.get("todo/1", Todo.class);
        ...
        Todo patch = Todo.builder()
                ...
                .completed(true)
                .build();
        Todo updated = client.request("todo/1").build("PATCH", Entity.json(patch)).invoke(Todo.class);
        ...
        WHEN("Remove record");
        client.delete("todo/1", Void.class);
        res = client.request("todo/").buildGet().invoke(new GenericType<>() {});
